...
const Career = () => {
    ...
    const fetchHistory = async (uid: string) => {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                console.warn("Failed to fetch history"); // Soft fail
                setMatches([]);
                return;
            }
    ...
    const getAvatarUrl = (path: string | null | undefined): string => {
        ...
        if (!path || !path.trim()) return `https://ui-avatars.com/api/?name=User&background=FF6B35&color=fff&size=128`;
        if (path.startsWith("/public/")) return `${backend}${path.replace("/public", "")}`;
        if (path.startsWith("/")) return `${backend}${path}`;
        return path.startsWith("http") ? path : `https://ui-avatars.com/api/?name=User&background=FF6B35&color=fff&size=128`;
    };
    ...
    return (
        <div className="bg-theme-bg-primary w-full h-screen flex overflow-hidden">
            ...
            <aside className="w-[300px] border-r-[1px] border-light border-opacity-[0.05] h-screen flex flex-col relative z-10 flex-shrink-0 hidden lg:flex">
                <Link to="/">
                    ...
                    {navigationItems.map((item, index) => (
                        <Link key={index} to={item.path}>
                            <div className="cursor-pointer flex items-center gap-3 px-3 py-2 w-full transition-all duration-150 hover:bg-white/5 rounded-lg">
                                ...
                                <span className={`font-questrial font-normal text-base whitespace-nowrap ${item.active ? "text-light" : "text-light/30"} transition-colors duration-150 text-white`}>
                                    {item.label}
                                </span>
                            </div>
                        </Link>
                ...
                <div className="mt-auto mb-[50px] px-[60px]">
                    <button onClick={handleLogout} className="w-full p-3 bg-transparent border border-solid border-[#f9f9f94c] rounded-[14px] flex items-center justify-center gap-2 cursor-pointer hover:bg-white/10 transition-all duration-150">
                        ...
                        <span className="text-light font-questrial text-white">Logout</span>
                    </button>
                </div>
            </aside>
            ...
            <div className="flex-1 flex h-full relative z-10 overflow-y-auto">
                <div className="w-full max-w-[1200px] mx-auto px-6 py-12">
                    <div className="mb-8">
                        <h1 className="font-questrial font-normal text-light text-4xl mb-2 text-white">My Career</h1>
                        <p className="font-questrial text-light/60 text-base text-white/60">History of your matches and performance.</p>
                    </div>
                    ...
                    {isLoading ? (
                        ...
                        <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-12 text-center">
                            ...
                            <h3 className="font-questrial text-light text-xl mb-2 text-white">No matches yet</h3>
                            <p className="font-questrial text-light/60 text-white/50">Play a game to start your career!</p>
                        </div>
                    ) : (
                                ...
                                <table className="w-full text-white">
                                    <thead>
                                        <tr className="border-b border-white/10">
                                            <th className="text-left py-4 px-6 font-questrial text-light/60 font-semibold text-white/70">Date</th>
                                            <th className="text-left py-4 px-6 font-questrial text-light/60 font-semibold text-white/70">Opponent</th>
                                            <th className="text-center py-4 px-6 font-questrial text-light/60 font-semibold text-white/70">Result</th>
                                            <th className="text-center py-4 px-6 font-questrial text-light/60 font-semibold text-white/70">Score</th>
                                            <th className="text-right py-4 px-6 font-questrial text-light/60 font-semibold text-white/70">XP</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {matches.map((match) => {
