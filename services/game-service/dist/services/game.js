...
export class GameServices {
    ...
    constructor() {
        ...
        this.matchmakingJoinTimes = new Map();
     ...
     */
    async handleScoreUpdate(userId, payload) {
     ...
     */
    async handleMatchEnd(initiatorId, payload) {
    ...
    async handlePlayerMove(userId, payload) {
     ...
     */
    async handleGameJoin(userId, payload) {
     ...
     */
    async handleMatchmaking(userId, payload) {
     ...
     */
    async joinExistingGame(userId, gameId) {
     ...
     */
    async joinMatchmaking(userId, gameType) {
     ...
     */
    async tryMatchPlayers(gameType) {
     ...
     */
    async createGameSession(player1Id, player2Id, gameType) {
     ...
     */
    startBotBehavior(gameSession, botId) {
     ...
     */
    async handleTournamentAction(userId, payload) {
     ...
     */
    async handleGameResult(userId, payload) {
     ...
     * @returns {Promise<any>} Result of the join operation.
     */
    async joinTournament(playerId, tournamentId, password) {
     ...
     */
    async startTournament(playerId, tournamentId) {
        const tournament = this.tournaments.get(tournamentId);
        if (!tournament) {
            throw new Error('Tournament not found');
        }
     ...
     */
    async startTournamentRound(tournamentId, round) {
        const tournament = this.tournaments.get(tournamentId);
        if (!tournament)
            return;
        ...
        for (const match of roundMatches) {
            if (match.player1 && match.player2) {
                ...
                const gameSession = await this.createTournamentGameSession(match.player1.id, match.player2.id, tournamentId, match.id);
     ...
     */
    async createTournamentGameSession(player1Id, player2Id, tournamentId, matchId) {
        const gameSession = {
            id: `game-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
            players: [player1Id, player2Id],
            gameType: 'tournament',
     ...
     */
    async processTournamentGameResult(tournamentId, result) {
        const tournament = this.tournaments.get(tournamentId);
        if (!tournament) {
            throw new Error('Tournament not found');
        }
        ...
        const loserId = gameSession.players.find((id) => id !== result.winnerId);
        if (loserId) {
     ...
     * @param {string} tournamentId - Tournament ID.
     */
    async advanceTournamentRound(tournamentId) {
        const tournament = this.tournaments.get(tournamentId);
        if (!tournament)
            return;
        const currentRoundMatches = tournament.bracket.filter(m => m.round === tournament.currentRound);
     ...
     * @param {string} tournamentId - The tournament ID.
     */
    forfeitActiveMatches(playerId, tournamentId) {
        const tournament = this.tournaments.get(tournamentId);
        if (!tournament)
            return;
        // Find active matches with this player
        const activeMatches = tournament.bracket.filter(m => m.status === 'in_progress' &&
     ...
     */
    sanitizeTournamentForClient(tournament) {
     ...
     */
    notifyTournamentPlayers(tournamentId, message) {
        const tournament = this.tournaments.get(tournamentId);
        if (tournament) {
            tournament.players.forEach(player => {
     ...
     */
    async advanceTournamentPlayer(tournamentId, winnerId) {
        const tournament = this.tournaments.get(tournamentId);
        if (!tournament)
            return;
